...
export class SSMHelper extends BaseClass implements ISSMHelper {
   ...
   */
  public async CancelCommandAsync(
    commandId: string,
  ): Promise<SSM.CancelCommandResult> {
    const action = `${SSMHelper.name}.${this.CancelCommandAsync.name}`;
    this.LogHelper.LogInputs(action, { commandId });

    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(commandId)) {
      throw new Error(`[${action}]-Must supply commandId`);
    }
   ...
   */
  public async DeleteParameterAsync(
    name: string,
   ...
   */
  public async GetParameterAsync(
    name: string,
   ...
   */
  public async GetParametersByPathAsync(
    path: string,
    nextToken?: string,
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(path)) {
      throw new Error(`[${action}]-Must supply path`);
    }
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(nextToken)) {
    ...
    if (response.NextToken && response.Parameters) {
      const nextParameters = await this.GetParametersByPathAsync(
        path,
        response.NextToken,
      );
   ...
   */
  public async PutParameterAsync(
    name: string,
    value: string,
    type: string,
    description: string,
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(value)) {
      throw new Error(`[${action}]-Must supply value`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(type)) {
      throw new Error(`[${action}]-Must supply type`);
    }
  ...
  public async SendCommandAsync(
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(documentName)) {
      throw new Error(`[${action}]-Must supply documentName`);
    }
    if (this.ObjectOperations.IsNullOrEmpty(parameters)) {
      throw new Error(`[${action}]-Must supply parameters`);
    }
    if (this.ObjectOperations.IsNullOrEmpty(instanceIds)) {
      throw new Error(`[${action}]-Must supply at least one instanceId`);
    }
    ...
    if (this.ObjectOperations.IsNullOrWhitespace(logGroupName)) {
      params.CloudWatchOutputConfig = {
